...
class GalaxyTest < Test::Unit::TestCase
  ...
  def test_galaxy
    ...
    history_name = "genome_Single-end_tophat2_"+current_time
    ...
    # genome_RNA-seq_01_Single-end(mapping in Tophat2) (imported from API)
    driver.switch_to.frame('galaxy_main')
    #element = driver.find_element(:id, "grid-table")
    element = driver.find_elements(:xpath, "//a[contains(.,'genome_RNA-seq_01_Single-end(mapping in Tophat2)')]")[0]
    element.click
    sleep 2
    count=count+1
    driver.save_screenshot("/work/galaxy-#{count}.png")
    ...
    #
    # Step 6: TopHat
    element = driver.find_elements(:xpath, "//span[contains(.,'Step 6: TopHat')]")[0]
    element.click
    sleep 2
    count=count+1
    driver.save_screenshot("/work/galaxy-#{count}.png")
