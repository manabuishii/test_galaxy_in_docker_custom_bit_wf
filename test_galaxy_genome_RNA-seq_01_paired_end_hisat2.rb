...
class GalaxyTest < Test::Unit::TestCase
  ...
  def test_galaxy
    ...
    history_name = "genome_Paired-end_hisat2_"+current_time
    ...
    # genome_RNA-seq_01_Paired-end(mapping in HISAT2) (imported from API)
    driver.switch_to.frame('galaxy_main')
    #element = driver.find_element(:id, "grid-table")
    element = driver.find_elements(:xpath, "//a[contains(.,'genome_RNA-seq_01_Paired-end(mapping in HISAT2)')]")[0]
    element.click
    sleep 2
    count=count+1
    driver.save_screenshot("/work/galaxy-#{count}.png")
