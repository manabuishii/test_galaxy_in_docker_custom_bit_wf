...
class GalaxyTest < Test::Unit::TestCase
  ...
  def test_galaxy
    ...
    history_name = "Paired-end_Sailfish_0.9_"+current_time
    ...
    # RNA-seq_01_Paired-end(Quantifying in Sailfish v0.9)(imported from API)
    driver.switch_to.frame('galaxy_main')
    #element = driver.find_element(:id, "grid-table")
    element = driver.find_elements(:xpath, "//a[contains(.,'RNA-seq_01_Paired-end(Quantifying in Sailfish v0.9)')]")[0]
    element.click
    sleep 2
    count=count+1
    driver.save_screenshot("/work/galaxy-#{count}.png")
    ...
    element = driver.find_elements(:xpath, "//span[contains(.,'Step 7: Sailfish_v0.9_Wrapper')]")[0]
    element.click
    sleep 2
    count=count+1
    driver.save_screenshot("/work/galaxy-#{count}.png")
