...
class GalaxyTest < Test::Unit::TestCase
  ...
  def test_galaxy
    ...
    history_name = "genome_Paired-end_tophat2_"+current_time
    ...
    datasets=["ES_SRR616819_div100_1.fastq", "ES_SRR616820_div100_1.fastq", "ES_SRR616821_div100_1.fastq", "ES_SRR616822_div100_1.fastq",
              "ES_SRR616819_div100_2.fastq", "ES_SRR616820_div100_2.fastq", "ES_SRR616821_div100_2.fastq", "ES_SRR616822_div100_2.fastq"
    ]
    ...
    driver.switch_to.frame('galaxy_main')
    element = driver.find_element(:id, "grid-table")
    element = element.find_elements(:xpath, "//a[contains(.,'mousegenome')]")[0]
    element.click
    sleep 2
    count=count+1
    driver.save_screenshot("/work/galaxy-#{count}.png")
    ...
    datasets=["Mouse_UCSC_mm10_Ensembl.bed"]
    datasets.each { |dataset|
      element.find_elements(:xpath, "//a[contains(.,'#{dataset}')]/../../input")[0].click
    }
    count=count+1
    ...
    # genome_RNA-seq_01_Paired-end(mapping in Tophat2) (imported from API)
    driver.switch_to.frame('galaxy_main')
    #element = driver.find_element(:id, "grid-table")
    element = driver.find_elements(:xpath, "//a[contains(.,'genome_RNA-seq_01_Paired-end(mapping in Tophat2)')]")[0]
    element.click
    sleep 2
    count=count+1
    driver.save_screenshot("/work/galaxy-#{count}.png")
    ...
    # Input Dataset [Paired end left]
    element = driver.find_elements(:xpath, "//label[contains(.,'Input Dataset [Paired end left]')]")[0]
    ...
    # Input Dataset [Paired end right]
    element = driver.find_elements(:xpath, "//label[contains(.,'Input Dataset [Paired end right]')]")[0]
    element2 = element.find_elements(:xpath, "./span[contains(@class,'multiinput')]")[0]
    element2.click
    # element = driver.find_elements(:xpath, "//label[contains(.,'Input Dataset [Single end reads]')]/span[contains(@class,'multiinput')]")[0]
    # p element.attribute('innerHTML')
    ...
    #
    # Step 4
    # Input Dataset [BED format]
    element = driver.find_elements(:xpath, "//label[contains(.,'Input Dataset [BED format]')]/..//select")[0]
    options=element.find_elements(:tag_name => "option")

    #### Select Mouse_UCSC_mm10_Ensembl.bed
    options.each do |g|
      if g.text.index("Mouse_UCSC_mm10_Ensembl.bed") != nil
        g.click
        break
      end
    end
    ...
    #
    # Step 8: TopHat
    element = driver.find_elements(:xpath, "//span[contains(.,'Step 8: TopHat')]")[0]
    element.click
    sleep 2
    count=count+1
    driver.save_screenshot("/work/galaxy-#{count}.png")
    # Select a reference genome
    element = driver.find_elements(:xpath, "//label[contains(.,'Select a reference genome')]/..")[0]
    # open editable_field
    ...
    #### Select Mus_musculus_UCSC_mm10
    options.each do |g|
      if g.text.index("Mus_musculus_UCSC_mm10") != nil
        g.click
        break
      end
    end
